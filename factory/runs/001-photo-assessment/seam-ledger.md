...
| 14 | 800-infra | 900-security | `docs/800-infra/04-ci-cd.md` — **the file exists and is not in this role's `reads:` block** | `under-supply` | **S-901-1, and the sharpest wiring finding of this role call.** 900's own contract tells it to review the deploy surface. It was given `00-environments.md` and `01-iac-plan.md` and not the file that holds the workflow triggers, the two OpenID Connect roles' trust policies, and what each of them may deploy. So it could not check whether a public repository can build an AWS environment. It scored T-27 / R-05 at **15, Critical, because the fact is unverified** — not because it found something wrong. That is the correct behaviour and it is also a wasted Critical. `04-ci-cd.md` should be added to `900-security`'s `reads:` in the handoff map |
| 15 | 400-architecture | 900-security | ADR-0003, and `docs/500-engineering/03-api-spec.md` §3 | `under-supply` | **S-901-2.** Nothing in any input says that `GET /api/auth/callback` compares the `state` value in the query with the `state` value in the cookie and refuses when they differ. The cookie's own attributes are written down in full; the comparison that the cookie exists to make is not. Without it the flow is open to authorization code injection. Traced to 400 rather than 500, because ADR-0003 is where the sign-in flow is decided and 500 wrote the route from it |
| 16 | 500-engineering | 900-security | `docs/500-engineering/03-api-spec.md` §4a | `under-supply` | **S-901-3.** The spec does not say whether the browser reuses the same `requestId` after an assessment fails. Both readings break something. If it reuses it, the `IDEM#` row is still there and the person's own next tap is refused for ten minutes — which breaks "the person taps again", the rule that replaced the automatic retry on 2026-08-26. If it makes a new one, the double-charge that the row was added to stop comes back |
| 17 | 400-architecture | 900-security | ADR-0008, and `docs/500-engineering/03-api-spec.md` §6 | `under-supply` | **S-901-4.** §6 says every `CONFIG` value is checked "against a closed list or a range", and no file names the range for `DAILY_LIMIT`. A value of 10,000 typed by hand would pass a check that does not exist, and cost about $40 in a day against a $5 balance |
| 18 | 800-infra | 900-security | `docs/800-infra/01-iac-plan.md` §4.3 | `under-supply` | **S-901-5.** §4.3 says the password policy is "Cognito's own" and no input says what that is. No file anywhere mentions multi-factor sign-in for the product's own users. The account password is the strongest key to one person's photos and its strength is unstated |
| 19 | 800-infra | 900-security | `docs/800-infra/00-environments.md` §4 | `under-supply` | **S-901-6.** No file says what happens when `LLM_PROVIDER` is missing or misspelled. If the fallback is the real adapter rather than the stub, an ordinary pull request preview can spend the $5 model balance |
| 20 | no upstream role | 900-security | no file — **no application code exists yet** | `clean, open` | **S-901-7.** The contract tells this role to name the file where the `USER` / `ADMIN` check runs server-side. It verified the rule itself — all 13 routes carry a decorator, the three anonymous ones are correct — and could not name a path, because nothing is built. It refused to invent one. Recorded as `clean` and not `under-supply`: the fact does not exist yet rather than having been withheld. The first task that writes the guard records the path in `02-mitigations.md` §3.1 |
| 21 | run input | 900-security | `factory/runs/001-photo-assessment/human-gates.md` — **not in this role's `reads:` block, and deliberately not published** | `under-supply` | **S-901-8.** Every residual risk needs a named, reachable person. The owner is the only person on the project and no readable input carries a name or a contact. This one may stay a seam on purpose — gate 58 keeps the owner's address out of the repository, so the fix is a file the roles cannot read, not a file they can |
